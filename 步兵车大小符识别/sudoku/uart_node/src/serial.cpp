...
void TransformData(int &fd,int number,int position){
	...
	transform[0] = 0xA5;
	transform[1] = (unsigned char)number;
	transform[2] = (unsigned char)position;
	transform[3] = 0xFF;//(unsigned char)(transform[0]+transform[1]+transform[2]);
    write(fd,transform,4);
}
